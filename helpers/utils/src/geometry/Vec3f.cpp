...
float Vec3f::x() const { return _x; }

float Vec3f::y() const { return _y; }

float Vec3f::z() const { return _z; }

Vec3f Vec3f::operator-() const { return Vec3f(-_x, -_y, -_z); }
...
float Vec3f::lengthSquared() const { return (_x * _x) + (_y * _y) + (_z * _z); }

void Vec3f::normalize() { *this /= length(); }
...
Vec3f operator*(const Vec3f &vec, float scalar) { return scalar * vec; }

Vec3f operator/(const Vec3f &v, float scalar) { return (1.0f / scalar) * v; }
...
Vec3f unitVector(const Vec3f &vec) { return vec / vec.length(); }
